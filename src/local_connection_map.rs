...
// register_map_entry will add a Sender to the map with a given name.
pub fn register_map_entry(name: String, sender:  Sender<FlowMessage>) {
    LOCAL_MAP.lock().unwrap().entry(name).or_insert(sender);
}

// get_map_entry will retrieve a sender that matches a name or NONE.
pub fn get_map_entry(name: String) -> Option<Sender<FlowMessage>> {
    if LOCAL_MAP.lock().unwrap().contains_key(&name) {
        return Some(LOCAL_MAP.lock().unwrap().get(&name).unwrap().clone());
    }
    None
}
// remove_map_entry will remove an entry
pub fn remove_map_entry(name: String) {
...
mod tests {
    ...
    fn map_test() {
        ...
        register_map_entry(thread1_id.clone(), tx1.clone());
        register_map_entry(thread2_id.clone(), tx2.clone());
        ...
        let map_entry_op2 = get_map_entry(thread2_id.clone());
        if map_entry_op2.is_some() {
            let sender2 = map_entry_op2.unwrap();
        ...
        remove_map_entry(thread1_id.clone());
        assert_eq!(num_thread_entries, LOCAL_MAP.lock().unwrap().len() + 1);
        remove_map_entry(thread2_id.clone());
