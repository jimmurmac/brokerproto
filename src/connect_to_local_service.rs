...
impl ServiceConnection for LocalServiceConnection {
    ...
    fn send(&self, msg: FlowMessage) -> Result<Option<FlowMessage>, ConnectionError> {
        if self.request_sender.is_none() {
            println!("No request sender available for service: {}", self.service_name);
        ...
        let send_result = self.request_sender.as_ref().unwrap().send(wrapped_msg.get_request());
        if send_result.is_err() {
            println!("Failed to send message to service: {}", self.service_name);
            return Err(ConnectionError::Connection(format!("Failed to send message")));
        }
        ...
        if let ConnectionStyle::SendReceive = self.connection_style {
            if self.response_receiver.is_none() {
                println!("No response receiver available for service: {}", self.service_name);
                return Err(ConnectionError::ServiceNotFound(self.service_name.clone()));
            }
            ...
            match response_receiver.recv() {
                ...
                Err(e) => a_result = Err(ConnectionError::Connection(format!("Failed to receive response: {}", e))),
            }
        }
    ...
    fn get_connection_style(&self) -> ConnectionStyle{ self.connection_style}
    fn get_connection_type(&self) -> ConnectionType{ self.connection_type}
    fn get_service_name(&self) -> String{ self.service_name.clone()}
...
pub fn connect_to_local_service(service_name: String, connection_type: ConnectionType, 
        connection_style: ConnectionStyle) -> Result<Box<dyn ServiceConnection>, 
        ConnectionError> 
{
        if  ConnectionType::Local != connection_type {
            println!("Invalid connection type specified for local service: {}", service_name);
            return Err(ConnectionError::URLError);
        }

        let service_connection = 
            LocalServiceConnection::new(service_name, connection_style);
